<template>
  <div style="padding: 24px;margin-top: 16px;background-color: white">
    <el-tabs v-model="activeName"  @tabChange="handleTabChange">
      <el-tab-pane label="基础资源配置" :name="'基础资源配置'"></el-tab-pane>
      <el-tab-pane label="租户默认配置" :name="'租户默认配置'"></el-tab-pane>
    ...
    <div class="gva-table-box">
      ...
      <div  style="padding: 16px 0">
        <warning-bar v-if="activeName==='基础资源配置'" title="为基础资源（宿主机）设置告警策略" />
      </div>
      <!-- 表格展示 -->
      <el-table
          :data="configList"
      ...
      >
        <template #header>
          <div class="flex justify-between items-center">
            <span class="text-lg">{{ IsCreated? '新增' : '编辑' }}{{ activeName }}</span>
            <div>
...
<!--          </el-form-item>-->
          ...
          <el-form-item label="一般告警阈值" prop="commonAlertData">
            <el-input v-if="dataInfo.monitorType>3&&dataInfo.monitorType<10" v-model="dataInfo.commonAlertData" type="number" :max="999999" :min="0" >
              <template #append>
                {{ "Mbps" }}
              </template>
            </el-input>
            <el-select v-else v-model="dataInfo.commonAlertData">
          ...
          <el-form-item label="严重告警阈值" prop="seriesAlertData">
            <el-input v-if="dataInfo.monitorType>3&&dataInfo.monitorType<10" v-model="dataInfo.seriesAlertData" type="number" :max="999999" :min="0">
              <template #append>
                {{ "Mbps" }}
              </template>
            </el-input>
            <el-select v-else v-model="dataInfo.seriesAlertData">
              <template v-if="dataInfo.monitorType===12">
                <el-option v-for="threshold in temperatureOptions" :key="threshold" :label="threshold + '&deg;C'" :value="threshold" />
              </template>
              <template v-else>
          ...
          <!--            </el-select>-->
          <!--          </el-form-item>-->
          <el-form-item label="通知方式" prop="alertWay">
  ...
  methods: {
    ...
    handleTabChange() {
      switch (this.activeName) {
        case "基础资源配置":
          this.searchInfo.alertConfigType = 3;
          break;
        case "租户默认配置":
    ...
    //监控项显示
    IsShowOption(val){
      if (val > 9&&this.activeName==="基础资源配置") {
        return false;
      }
      const tempData=this.configList.filter(item=>item.monitorType==val)
    ...
    handleEdit(val) {
      ...
      // 转换用户类别值
      this.dataInfo.alertUser = val.alertUser.map(String);
      // 转换通知方式值
    ...
    AddConfig() {
      this.$refs.dataInfo.validate((valid) => {
        if (valid) {
          ...
          if (this.activeName=== '基础资源配置') {
            params.alertConfigType=3
          }else{
            params.alertConfigType=1
          }
