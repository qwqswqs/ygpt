    ...
    <div class="gva-filter-box" style="padding-top: 10px">
      ...
      >
        ...
        <el-form-item label="资源类别">
          <el-select style="width: 240px" v-model="searchInfo.resourceType" clearable>
            <el-option label="裸金属" :value="1"></el-option>
            <el-option label="云主机" :value="2"></el-option>
            <el-option label="容器" :value="3"></el-option>
            <el-option label="宿主机" :value="4"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="告警类别">
          <el-select style="width: 240px" v-model="searchInfo.alertType" clearable>
...
import {getSystemAlertList, updateSystemAlert} from "@/api/yunguan/config/alertInfo";
...
const handleView = (row) => {
  updateSystemAlert({id:row.ID}).then(res=>{
    if (res.code==0){
      ElMessage({
        type:'success',
        message:'已查看'
...
const resourceType=ref(["裸金属", "云主机", "容器", "宿主机"]);
const alertType={
  1:'CPU使用率',
  2:'内存使用率',
  3:'磁盘使用率',
