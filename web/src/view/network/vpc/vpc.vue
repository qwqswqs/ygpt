    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column label="IP地址">
          <template #default="scope">
            <span>{{GetIpName (scope.row.ipID) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="访问规则">
          <template #default="scope">
            <span>{{ scope.row.accessRule }}</span>
          </template>
        </el-table-column>
        <el-table-column label="路由规则">
          <template #default="scope">
            <span>{{ scope.row.routerRule }}</span>
          </template>
        </el-table-column>
        <el-table-column label="安全规则">
          <template #default="scope">
            <span>{{ scope.row.safeRule }}</span>
          </template>
        </el-table-column>
        <el-table-column label="管理人员">
          <template #default="scope">
        ...
        <el-table-column label="归属单位">
          <template #default="scope">
            <span>{{ scope.row.belongLocation }}</span>
          </template>
        </el-table-column>
      ...
      >
        ...
        >
          ...
          <el-form-item label="区域" prop="zone">
            <el-input v-model="dataInfo.zone"/>
          </el-form-item>
          <el-form-item label="IP地址" prop="ipID">
            <el-select v-model="dataInfo.ipID" placeholder="请选择归属类别">
              <el-option
          ...
          <el-form-item label="访问规则" prop="accessRule">
            <el-input v-model="dataInfo.accessRule"/>
          </el-form-item>
          <el-form-item label="路由规则" prop="routerRule">
            <el-input v-model="dataInfo.routerRule"/>
          </el-form-item>
          <el-form-item label="安全规则" prop="safeRule">
            <el-input v-model="dataInfo.safeRule"/>
          </el-form-item>
          <el-form-item label="管理人员" prop="manager">
            <el-input v-model="dataInfo.manager"/>
          </el-form-item>
          <el-form-item label="归属单位" prop="belongLocation">
            <el-input v-model="dataInfo.belongLocation"/>
          </el-form-item>
...
<script>
import {addVpc, deleteVpc, getVpcList, updateVpc} from "@/api/yunguan/network/netVpc";
...
export default {
  ...
  methods: {
    ...
    UpdateVpc() {
      updateVpc(this.dataInfo).then(response => {
        if (response.code == 0) {
          this.$message({
            message: '编辑成功',
            type: 'success'
    ...
    AddVpc() {
      this.$refs.dataInfo.validate(valid => {
        if (valid) {
          addVpc(this.dataInfo).then(response => {
            if (response.code == 0) {
              this.$message({
                message: '添加成功',
                type: 'success'
