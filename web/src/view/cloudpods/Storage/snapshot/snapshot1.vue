  ...
  <div style="background-color: white">
    <div class="gva-search-box" style="margin-top:10px">
      <div class="gva-btn-list">
        <el-form label-width="70px" :inline="true" :model="searchInfo" style="display: flex;justify-content: space-between">
          <div>
          ...
          <el-form-item label="状态">
            <el-select v-model="searchInfo.status" style="width: 180px;" placeholder="请选择" clearable>
              <el-option label="可用" value="ready"/>
              <el-option label="故障" value="fail"/>
            </el-select>
          </el-form-item>
          <el-form-item label="CPU架构">
            <el-select style="width: 180px;" v-model="searchInfo.osArch" placeholder="请选择" clearable>
          ...
          <el-form-item>
            ...
            <el-button class="button-gap" type="info" @click="searchInfo.name='';searchInfo.osArch='';GetSnapshotList()">重置</el-button>
          </el-form-item>
    ...
    <div class="gva-table-box">
      <div style="margin-bottom: 16px;">
        <el-button :disabled="!ids.length" type="grey" @click="HandleDeleteSnapshotByIds">
          删除
        </el-button>
      </div>
      <el-table
      ...
      >
        ...
        <el-table-column label="云主机" width="200">
          <template #default="scope">
            ...
            <i v-else-if="scope.row.guest_status=='ready'" class="status-dot" style="background-color: rgb(174,185,192)"/>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-tooltip :disabled="scope.row.guest_status=='ready'" content="云主机必须在关机状态下才可进行此操作"
                        placement="top">
              <el-button link type="text" @click="HandleSnapshotReset(scope.row)"
                         :disabled="scope.row.guest_status!='ready'">回滚主机
              </el-button>
            </el-tooltip>
            <el-button link type="text" @click="HandleDeleteSnapshot(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  methods: {
    ...
    HandleDeleteSnapshot(val) {
      this.$confirm('此操作将永久删除该快照, 是否继续?', '提示', {
        ...
        deleteSnapshot({snapshotId: val.id}).then(response => {
          if (response.code == 0) {
            this.$message({
              message: '删除成功',
              type: 'success'
