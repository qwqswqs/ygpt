<template>
  <div>
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button type="text" @click="DeleteBackInfo(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      ...
      <input type="file" ref="file" @change="getFileData"/>
    </el-drawer>
  </div>
</template>
<script>
import {addBack,deleteBack, getBackList} from "@/api/yunguan/backup/backup";
...
export default {
  ...
  methods: {
    ...
    AddBackInfo() {
      addBack(this.storage).then(response => {
        if (response.code == 0) {
          this.$message({
            message: '上传成功',
            type: 'success'
