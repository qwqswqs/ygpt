    ...
    <el-drawer
      ...
      >
        <el-form-item label="名称" prop="name">
          <el-input v-model="dataInfo.name" placeholder="请输入数据集名称"/>
        </el-form-item>
        <el-form-item label="标签" prop="usage">
          <el-select v-model="dataInfo.usage" multiple placeholder="请选择数据集标签" style="width: 100%">
            <el-option
        ...
        <el-form-item label="描述" prop="description">
          <el-input v-model="dataInfo.description" placeholder="请描述数据集用途"/>
        </el-form-item>
...
export default {
  ...
  created() {
    this.GetMyDatasetList()
    this.GetShareDatasetList()
  },
  methods: {
    ...
    handleShare(val) {
      ...
      updateElement(obj).then(response => {
        if (response.code == 0) {
          ...
          this.GetMyDatasetList(this.searchInfo)
          this.GetShareDatasetList(this.searchInfo)
          this.dialogVisible = false
        } else {
          this.$message({
            message: '修改失败',
