<template>
  <div style="padding: 24px;margin-top: 16px;background-color: white">
    ...
    <div class="gva-table-box">
      <div class="gva-search-box" style="margin-top: 16px">
        <div class="gva-btn-list">
          <el-form ref="searchForm" :inline="true" :model="searchInfo" label-width="70px"
      ...
      <div class="gva-table-box">
        <div style="margin-bottom: 16px">
          ...
          <el-button :disabled="!selections.length" type="grey" @click="openDeleteDialog('batch')">
            删除
          </el-button>
        </div>
        <!-- 表格具体内容 -->
        <div v-if="tableData && tableData.length > 0">
          <el-table :data="tableData" @selection-change="handleSelectionChange"
            ...
            <el-table-column align="left" fixed label="序号" min-width="100" prop="num" type="index"/>
            <el-table-column align="left" class="text-overflow" label="标题" min-width="100" prop="product.name">
            ...
            <!--          <el-table-column align="left" class="text-overflow" label="文件" min-width="200" prop="product.name">-->
            <!--            <template #default="scope">-->
            <!--              <div class="text-overflow text-details" @click="handleFilesClick(scope.row)">-->
            <!--                文件列表-->
            <!--              </div>-->
            <!--            </template>-->
            <!--          </el-table-column>-->
            ...
            <el-table-column align="left" label="描述" min-width="150" prop="product.description">
              <template #default="scope">
            ...
            <el-table-column align="left" fixed="right" label="操作" width="300">
              <template #default="scope">
                <el-button link :disabled=checkDownloadIsAvailable(scope.row) @click="OpenDownDialog(scope.row)  ">
                  下载
                </el-button>
                <el-button link @click="editFunc(scope.row)">
                  编辑
                </el-button>
                <el-button link  @click="openDeleteDialog('single', scope.row)" >
                  删除
                </el-button>
              </template>
            </el-table-column>
    ...
    <!-- 抽屉：增加/编辑 -->
    <el-drawer v-model="dialogFormVisible" :before-close="closeDialog" :show-close="false" size="60%">
      <div class="flex justify-between items-center" style="margin-bottom: 24px">
        <span class="text-lg">{{ dialogTitle }}AI产品</span>
        <el-icon
      ...
      <el-form ref="form" :model="editForm" :rules="editRules" label-width="100px">
        <!-- 基本信息 -->
        <el-form-item label="产品类型" label-width="90px" prop="productType">
        ...
        <el-form-item label="产品名称" label-width="80px" style="padding-left: 10px" prop="product.name">
        ...
        <el-form-item v-if="secondConfigList.length > 0" label="二级类型" label-width="90px" prop="secondType">
          <el-select style="width: 320px" v-model="editForm.secondType" placeholder="选择二级类型"
        ...
        <!--        <el-form-item label="来源" label-width="80px" style="padding-left: 10px" prop="product.source">-->
        ...
        <!--          </el-radio-group>-->
        <!--        </el-form-item>-->
        <!--        <el-form-item label="开放类别" label-width="80px" style="padding-left: 10px" prop="product.openType">-->
        ...
        <!--          </el-select>-->
        <!--        </el-form-item>-->
        <!--        <el-form-item label="定制价格" label-width="80px" style="padding-left: 10px" prop="product.price">-->
        <!--          <el-input-number-->
        ...
        <!--          />-->
        <!--        </el-form-item>-->
        <!--        <el-form-item label="可否下载" label-width="80px" style="padding-left: 10px" prop="product.isDownload">-->
        ...
        <div v-if="dialogTitle==='新增'">
          <el-form-item label="上传类型" label-width="80px" style="padding-left: 10px" prop="uploadType">
            <div style="display: flex;background: rgba(242, 243, 245, 1);">
              <el-button
          ...
          <el-form-item v-if="editForm.uploadType===1" label-width="90px" label="附件"
                        prop="fileName">
            <el-upload
                :action="uploadUrl"
          ...
          <el-form-item v-if="editForm.uploadType===2" label-width="90px" label="文件链接"
                        prop="url">
        ...
        <div v-else-if="dialogTitle==='编辑'||dialogTitle==='详情'">
          <el-form-item v-if="editForm.productFiles!==''&&editForm.productFiles!=null">
            <!-- 基本信息 -->
            <div v-for="(product, productIndex) in editForm.productFiles" :key="productIndex"
        ...
        <el-form-item label="详细描述" label-width="80px" style="padding-left: 10px" prop="product.description">
          <el-input
    ...
    <el-dialog
        ...
        width="40%"
    >
      <el-tabs v-model="instanceType" @tab-change="handleTabsClick">
...
const openDeleteDialog = (type, row = null) => {
  ...
  if (type === 'single' && row) {
    ...
    selectedIds.value = selections.value.map(item => item.product.id);
...
const instanceType = ref('baremetal');
...
const modelRules = ref({
  privateIp: [{required: true, message: '请选择实例', trigger: 'blur'}],
});
...
const downReqRef = ref(null);

const checkDownloadIsAvailable = (row) => {
  if (!row.productFiles.length) return true
  return (row.productFiles[0].name === "" && row.productFiles[0].url !== "");
}

const handleTabsClick = () => {
  if (instanceType.value === 'baremetal') {
    instanceList.value = allInstanceList.value.bare_metalList;
    instanceInfo.value.type = 1;
  } else if (instanceType.value === 'virtual') {
    instanceList.value = allInstanceList.value.virtualList;
    instanceInfo.value.type = 2;
  } else if (instanceType.value === 'container') {
    instanceList.value = allInstanceList.value.containerList;
    instanceInfo.value.type = 3;
  }
  downloadReq.value.privateIp = '';
}

const GetRenterInfoList = async () => {
  instanceInfo.value.type = 1;
  getRenterResList(instanceInfo.value).then(response => {
    if (response.code === 0) {
      allInstanceList.value.bare_metalList = response.data.list.filter(item => item.status === "running")
    }
  })
  instanceInfo.value.type = 2;
  getRenterResList(instanceInfo.value).then(response => {
    if (response.code === 0) {
      allInstanceList.value.virtualList = response.data.list.filter(item => item.status === "running")
    }
  })
  instanceInfo.value.type = 3;
  getRenterResList(instanceInfo.value).then(response => {
    if (response.code === 0) {
      allInstanceList.value.containerList = response.data.list.filter(item => item.status === "Running")
    }
  })
}

const OpenDownDialog = async (row) => {
  //重置
  downloadReq.value = {
    privateIp: '',
    DownloadDetails: []
  ...
  // 等待异步数据加载完成
  await GetRenterInfoList();
  downDetail.value.ProductType = row.product.subtype==5?1:2;
  downDetail.value.ProductName = row.product.name;
  downDetail.value.productId = row.product.id;
  downDetail.value.FileName = row.productFiles[0].name;
  downloadReq.value.DownloadDetails.push(downDetail.value)
  downloadReq.value.privateIp = '';
  downDialog.value = true;
}

const HandleDownloadElem = async () => {
  if (downloadReq.value.privateIp === '') {
    ElMessage.error('请选择实例');
    return;
  }
  try {
    const response = await DownloadElement(downloadReq.value);
    if (response.code === 0) {
...
const handleSecondClick = async () => {
  ...
  console.log("type" + editForm.value.secondType)
};
...
const editRules = ref({
  ...
  type: [{required: true, message: "请选择标签", trigger: "blur"}],
  subType: [{required: true, message: "请选择标签", trigger: "blur"}],
  secondType: [{required: true, message: "请选择标签", trigger: "blur"}],
  thirdType: [{required: true, message: "请选择标签", trigger: "blur"}],
  title: [{required: true, message: "请输入标题", trigger: "blur"}],
  price: [{required: true, message: "价格不能为空", trigger: "blur"}],
  isDownload: [{required: true, message: "请选择", trigger: "blur"}],
...
const getAllTableData = async () => {
  ...
  if (searchInfo.value.eleType !== "") {
    params.eleType = searchInfo.value.eleType;
  }
  if (searchInfo.value.subType !== "") {
    params.subType = searchInfo.value.subType;
  }
  ...
  const table = await getUserSupplyList(params);
...
const confirmProduct = async () => {
  if (form.value) {
    ...
    else res = await updateProduct(param);
    if (res.code === 0) {
      ElMessage({
        type: "success",
