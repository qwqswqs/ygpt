...
import (
	...
	systemRes "ygpt/server/model/system/response"
	"ygpt/server/utils"
...
func (a *AuthorityApi) GetAuthorityList(c *gin.Context) {
	...
	total, list, err := authorityService.GetAuthorityInfoList(pageInfo, authorityID)
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败"+err.Error(), c)
		return
	}
	response.OkWithDetailed(response.PageResult{List: list, Total: total}, "获取成功", c)
}
...
// 获取所有权限ID
func (a *AuthorityApi) GetDataAuthorityList(c *gin.Context) {
	data, err := authorityService.GetAllAuthorityList()
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败"+err.Error(), c)
		return
