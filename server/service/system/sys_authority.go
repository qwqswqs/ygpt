...
import (
	...
	"ygpt/server/model/system/response"
)
...
func (authorityService *AuthorityService) CreateAuthority(auth system.SysAuthority) (authority system.SysAuthority, err error) {
	auth.AuthorityId = uint(time.Now().Unix())
...
func (authorityService *AuthorityService) GetAuthorityInfoList(page request.PageInfo, authorityID uint) (num int64, list []system.SysAuthority, err error) {
	...
	if global.GVA_CONFIG.System.UseStrictAuth {
		...
		if *authority.ParentId == 0 {
			...
			db.Count(&total)
			err = db.Preload("DataAuthorityId").Where("authority_id = ?", authorityID).Offset((page.Page - 1) * page.PageSize).Limit(page.PageSize).Find(&authorities).Error
		} else {
			// 非顶级角色只能修改以下权限
			db.Count(&total)
			err = db.Debug().Preload("DataAuthorityId").Where("parent_id = ?", authorityID).Offset((page.Page - 1) * page.PageSize).Limit(page.PageSize).Find(&authorities).Error
		}
	} else {
		db.Count(&total)
		err = db.Preload("DataAuthorityId").Where("parent_id = ?", "0").Offset((page.Page - 1) * page.PageSize).Limit(page.PageSize).Find(&authorities).Error
	}

	for k := range authorities {
		err = authorityService.findChildrenAuthority(&authorities[k])
	}
	return total, authorities, err
}
...
// 获取所有权限
func (authorityService *AuthorityService) GetAllAuthorityList() (list interface{}, err error) {
	var authorities []system.SysAuthority
	db := global.GVA_DB.Model(&system.SysAuthority{})

	db = db.Where("authority_id != 777")
