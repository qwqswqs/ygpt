...
// 获取租户列表
func (userService *UserService) GetOpsUserInfoList(info systemReq.GetUserList) (list []response.GetOpsUserListRes, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	db := global.GVA_DB.Model(&system.SysUser{})
	var userList []response.GetOpsUserListRes
...
// 获取租户列表
func (userService *UserService) GetAllRenterInfoList() (list []response.GetOpsUserListRes, total int64, err error) {
	db := global.GVA_DB.Model(&system.SysUser{})
	var userList []response.GetOpsUserListRes
...
// 获取租户列表
func (userService *UserService) GetAllUserInfoList() (list []response.GetOpsUserListRes, total int64, err error) {
	db := global.GVA_DB.Model(&system.SysUser{})
	var userList []response.GetOpsUserListRes
...
// 修改登录时间
func (userService *UserService) UpdateOpsUserLoginTime(id uint) error {
...
//// 设置租户minio存储桶信息
//func (userService *UserService) SetUserMinioInfo(req system.SysUser) error {
//	return global.GVA_DB.Model(&system.SysUser{}).
//		Select("endpoint", "access_key", "secret_key", "sto_bucket_name", "bak_bucket_name").
//		Where("id=?", req.ID).
//		Updates(map[string]interface{}{
//			"endpoint":        req.Endpoint,
//			"access_key":      req.AccessKey,
//			"secret_key":      req.SecretKey,
//			"sto_bucket_name": req.StoBucketName,
//			"bak_bucket_name": req.BakBucketName,
//		}).Error
//}
