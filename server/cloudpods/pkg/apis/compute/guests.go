...
type GuestAutoRenewInput struct {
	...
	// 若公有云本身不支持自动续费, 则在本地周期(默认三小时)检查快过期云主机并进行续费,续费周期根据设置，请避免使用特殊的计费周期，避免续费失败
...
type ServerDeployInput struct {
	...
	// 部署完成后是否自动启动
	// 若云主机重置密码后需要重启生效，并且当前云主机状态为running, 此参数默认为true
	// 若云主机状态为ready, 指定此参数后，部署完成后，云主机会自动启动
	AutoStart bool `json:"auto_start"`
}
...
type GuestPerformStartInput struct {
	// 指定启动云主机的Qemu版本，可选值：2.12.1, 4.2.0
	// 仅适用于KVM云主机
