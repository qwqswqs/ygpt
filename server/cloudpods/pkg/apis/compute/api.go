...
type DiskConfig struct {
	...
	// 挂载到云主机的磁盘顺序, -1代表不挂载任何云主机
	// default: -1
	Index     int   `json:"index"`
	BootIndex *int8 `json:"boot_index"`

	// 镜像ID,通过镜像创建磁盘,创建云主机时第一块磁盘需要指定此参数
	// required: false
...
type ServerCreateInput struct {
	...
	*ServerConfigs
	...
	// 云主机内存大小,单位Mb,若未指定instance_type,此参数为必传项
	VmemSize       int  `json:"vmem_size"`
	EnableMemclean bool `json:"enable_memclean"`

	// 云主机Cpu大小,若未指定instance_type,此参数为必传项
	// default: 1
	...
	// 弹性公网IP名称或ID
	// 绑定已有弹性公网IP, 此参数会限制云主机再谈下公网IP所在的区域创建
	// required: false
	...
	// 使用主机快照创建云主机, 主机快照不会重置密码及秘钥信息
	// 使用主机快照创建的云主机将沿用之前的密码秘钥及安全组信息
	// required: false
	InstanceSnapshotId string `json:"instance_snapshot_id,omitempty"`

	// 使用主机备份创建云主机, 主机快照不会重置密码及秘钥信息
	// 使用主机备份创建的云主机将沿用之前的密码秘钥及安全组信息
	// required: false
