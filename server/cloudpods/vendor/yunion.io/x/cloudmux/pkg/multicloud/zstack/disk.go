...
func (disk *SDisk) GetStatus() string {
	...
	case "NotInstantiated":
		//数据云盘特有的状态。在这个连接状态中，数据云盘只存在于数据库的表记录中。NotInstantiated状态的数据云盘可以挂载到任何类型云主机管理程序管理的云主机上；当挂载到云主机上后，数据云盘的hypervisorType域会存储云主机对应的云主机管理程序类型，在主存储上被实例化为云主机管理程序类型的实际二进制文件，同时连接状态会改为就绪（Ready）；在这之后，这些数据云盘就只能被重新挂载到相同类型云主机管理程序管理的云主机上了。
